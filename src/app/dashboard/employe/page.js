...
import { EmployeeDetailsTable,EmployeeTableColumns } from '@/app/constants/PageDummyDatas/EmployeeDetailsTable';
 const EmployeeDetails = ({path}) => {
   const inputClass = "border border-gray-300 rounded-md p-2";
          ...
          const [filteredData, setFilteredData] = useState(EmployeeDetailsTable);
        ...
        const handleExportSelect = (value) => {
          if (value === 'print') {
            ...
            const exportData = EmployeeDetailsTable;
    ...
    const handleClear = () => {
      ...
      setFilteredData(EmployeeDetailsTable);
    };
     //filter
     const handleSearch = () => {
      const filtered = EmployeeDetailsTable.filter(item => {
...
<div className='flex justify-between items-center'>
  ...
  { label: 'XSL', value: 'xsl' },
    ...
    {/* check till here */}
<Button label="Add Employee" path="/MainLayout/AddEmployeeForm"/>
</div>
</div>
    ...
    <div>
     ...
     <div className="flex justify-center">
      <TableComponent columns={EmployeeTableColumns} data={filteredData}/>
     </div>
     </div>
    </div>
  );
