...
import { StatesDetailsTable, StatesTableCoums} from '@/app/constants/settingManusData/StatesDetailsTable';
import {  Button, TableComponent, DropdownButton, NormalButton,FilterInput } from '@/app/components';
 const State = ({ path }) => {
  ...
  const [filteredData, setFilteredData] = useState(StatesDetailsTable);
  ...
  const handleSearch = () => {
    ...
    if (!hasAtLeastOneFilter) {
      setFilteredData(StatesDetailsTable);
      return;
    }

    const filtered = StatesDetailsTable.filter((item) => {
      return (
        (filters.name.trim() === '' ||
