...
import { CountriesDetailsTable,CountriesTableCoums } from '@/app/constants/settingManusData/CountriesDetailsTable';
import {  Button, TableComponent, DropdownButton, NormalButton,FilterInput } from '@/app/components';
 const Country = () => {
  ...
  const [filteredData, setFilteredData] = useState(CountriesDetailsTable);
  ...
  const handleSearch = () => {
    ...
    if (!hasAtLeastOneFilter) {
      setFilteredData(CountriesDetailsTable);
      return;
    }

    const filtered = CountriesDetailsTable.filter((item) => {
      return (
        (filters.name.trim() === '' ||
