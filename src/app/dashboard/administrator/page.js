...
import { AdministrtorDetailsTable,AdministrtorTableColumns } from '@/app/constants/PageDummyDatas/AdministrtorDetailsTable';
 const AdministrtorDetails = ({path}) => {
    const inputClass = "border border-gray-300 rounded-md p-2";
     ...
     const [filteredData, setFilteredData] = useState(AdministrtorDetailsTable);
    ...
    const handleExportSelect = (value) => {
      if (value === 'print') {
        ...
        if (AdministrtorDetailsTable.length === 0) {
          alert('No data to export!');
          return;
        }
  
        const worksheet = XLSX.utils.json_to_sheet(AdministrtorDetailsTable);
        const workbook = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(workbook, worksheet, 'administrator');
        ...
        saveAs(data, 'administrator.xlsx');
      }
    };
    //onchange
    ...
    const handleClear = () => {
      ...
      setFilteredData(AdministrtorDetailsTable);
    };
    //filter
    const handleSearch = () => {
      const filtered = AdministrtorDetailsTable.filter(item => {
  ...
  { label: 'XSL', value: 'xsl' },
    ...
    {/* check till here */}
<Button label='Add Adminitrator' path="/MainLayout/AddAdministratorForm"/>
</div>
</div>
    ...
    <div>
     ...
     <div className="flex justify-center">
      <TableComponent columns={AdministrtorTableColumns} data={filteredData}/>
     </div>
     </div>
    </div>
  );
